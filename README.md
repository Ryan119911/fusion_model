...
### CoppeliaSim 中的负 z 压力坐标

`pose_refined.csv` 的 `z` 仍是 B-BSMG 姿态高度 `H`（11–20 mm），不能直接
作为 UR5 法兰的世界坐标。回放工具使用论文间的仿真桥梁，以 `H=20 mm` 为
零压力、`H=11 mm` 为最大压力，转换为 `[-0.015, 0] m` 的有符号压缩量。
负值表示虚拟毛笔受压，不表示刚性机械臂或法兰进入纸面下方。
...
默认虚拟笔长为 80 mm，最大压缩为 15 mm，因此接触书写时法兰仍至少在纸面
上方约 65 mm。输出目录的 `mapped_pressure_trajectory.csv` 同时保存
`original_h_mm` 与负的 `z`，防止姿态 H、压力坐标和机器人 TCP 坐标混淆。
这些长度仍是仿真参数，真实执行前必须用毛笔长度、TCP 与力—位移标定替换。
