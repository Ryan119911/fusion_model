...
class TrajectoryCatalogTest(unittest.TestCase):
    ...
    def test_sharded_v16_manifests_are_merged(self):
        with self.temporary_directory() as temporary:
            root = Path(temporary)
            records = []
            for character in ("一", "二"):
                output = root / character_directory_name(character)
                output.mkdir()
                (output / "pose_refined.csv").write_text(
                    "character,sample_id\n"
                    f"{character},{character}_fake_sim\n",
                    encoding="utf-8",
                )
                (output / "target.png").write_bytes(b"target")
                records.append(
                    {
                        ...
                        "status": "completed",
                        "output_dir": str(output),
                        "trajectory_sample_id": f"{character}_fake_sim",
                        "trajectory_point_count": 1,
                    }
                )
            for index, record in enumerate(records):
                (root / f"manifest_shard_{index}_of_2.jsonl").write_text(
                    json.dumps(record, ensure_ascii=False) + "\n",
                    encoding="utf-8",
                )

            catalog = TrajectoryCatalog(root)
            summary = catalog.summary()

            self.assertEqual(summary["ready_count"], 2)
            entry = catalog.resolve("二")
            self.assertIsNotNone(entry)
            assert entry is not None
            self.assertTrue(entry.ready)
            self.assertEqual(entry.quality, "v16_batch")
            self.assertEqual(entry.metadata["source"], "v16_pose_refined")
