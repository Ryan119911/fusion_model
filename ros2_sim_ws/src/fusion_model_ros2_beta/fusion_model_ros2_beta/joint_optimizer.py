"""Beta copy of model optim/paper_psoc_lm.py: actual gamma-response guard.

Only the equal-axis-scale guard differs from the upstream implementation.
Equal scale factors do not imply a rotationally symmetric neural footprint.
No upstream/CoppeliaSim file is changed by this module.
"""
...
def project_posture_decisions(vector, posture_count, epsilon):
    """Remove latent clipped-sigmoid plateaus, preserving physical bounds.

    Only H/alpha/beta node logits are projected. Gamma and XY are untouched.
    Applied inside optimization, never as an exported-CSV correction.
    """
    if not 0 < epsilon < .5 or not 0 <= posture_count <= vector.numel():
        raise ValueError('invalid posture projection domain')
    limit = float(np.log((1-epsilon)/epsilon))
    projected = vector.clone()
    projected[:posture_count] = projected[:posture_count].clamp(-limit, limit)
    return projected


def difference_column(evaluate, vector, column, step, base, scheme='forward'):
    """Bound-aware central option; neither option changes physical limits."""
    if scheme not in ('forward', 'central') or not np.isfinite(step) or step <= 0:
        raise ValueError('invalid difference scheme or step')
    plus = vector.clone()
    plus[column] += step
    high = evaluate(plus)
    if scheme == 'forward':
        return (high-base)/step
    minus = vector.clone()
    minus[column] -= step
    return (high-evaluate(minus))/(2*step)


def spatial_depth_residuals(xy, depth_mm, point_indices, weight, mm_per_pixel):
    """Arc-length depth energy: integral (dH/ds)^2 ds, within strokes.

    A 0.25 mm numerical floor handles repeated XY samples without division
    by zero. This is a soft shape penalty, NOT a calibrated speed limit.
    """
    if not np.isfinite(weight) or weight < 0 or not np.isfinite(mm_per_pixel) or mm_per_pixel <= 0:
        raise ValueError('invalid spatial depth penalty units or weight')
    if weight == 0:
        return []
    residuals = []
    for indices in point_indices:
        if len(indices) < 2:
            continue
        idx = torch.as_tensor(indices, dtype=torch.long, device=xy.device)
        points, h = xy[idx], depth_mm[idx]
        lengths = (torch.linalg.vector_norm(points[1:]-points[:-1], dim=-1)
                   * mm_per_pixel).clamp_min(.25)
        residuals.append(weight**.5 * (h[1:]-h[:-1]) / torch.sqrt(lengths))
    return residuals


def angle_point_residuals(angles, point_indices, weight):
    """Periodic decoded-angle continuity, never across stroke boundaries.

    Chord differences avoid treating +pi and -pi as a full-turn jump.
    This is an optimization penalty, not a physical angular-speed limit.
    """
    ...
    for indices in point_indices:
        if len(indices) < 2:
            continue
        values = angles[torch.as_tensor(indices, dtype=torch.long, device=angles.device)]
        residuals.extend([
            weight**0.5 * (torch.sin(values[1:])-torch.sin(values[:-1])).flatten(),
            weight**0.5 * (torch.cos(values[1:])-torch.cos(values[:-1])).flatten(),
        ])
    return residuals
...
class PaperPSOCLM:
    ...
    def __init__(
        ...
        if self.hard_neural_domain and (not optimize_gamma or neural_domain_weight <= 0 or render_stride != 1):
            raise ValueError('hard neural domain requires gamma, positive domain weight and full render sampling')
        self.project_posture_logits = bool(project_posture_logits)
        ...
        if not np.isfinite(depth_spatial_weight) or depth_spatial_weight < 0 or not np.isfinite(depth_mm_per_pixel) or depth_mm_per_pixel <= 0:
            raise ValueError('invalid spatial depth penalty units or weight')
        self.depth_spatial_weight = float(depth_spatial_weight)
        self.depth_mm_per_pixel = float(depth_mm_per_pixel)
        ...
        if self.optimize_gamma and np.isclose(
            ...
            # Test the actual forward response, not scaling anisotropy.
            device = next(renderer.bbsmg.parameters()).device
            center = float(renderer.image_size)/2
            xy = torch.tensor([[center-4,center], [center,center], [center+4,center]],
                              dtype=torch.float32, device=device)
            posture = torch.tensor([[11.,.04,.02], [15.,.04,.02], [20.,.04,.02]],
                                   dtype=torch.float32, device=device)
            strokes = torch.zeros(3, dtype=torch.long, device=device)
            with torch.no_grad():
                initial = renderer(xy, posture, strokes, torch.zeros(3,device=device))
                rotated = renderer(xy, posture, strokes, torch.full((3,),.2,device=device))
                gamma_response = float(torch.linalg.vector_norm(rotated-initial).cpu())
            if not np.isfinite(gamma_response) or gamma_response <= 1e-5:
                raise ValueError('Gamma has no usable forward response at equal metric scales')
            print(f'[BETA GAMMA GUARD] equal metric scales; actual response L2={gamma_response:.8g}; '
                  'nonzero response is not proof of unique pose recovery', flush=True)
    ...
    def optimize(
        ...
        if self.hard_neural_domain and gamma_decision_count:
            decoded_gamma, _ = self._decode_gamma(
                decision[posture_decision_count:posture_decision_count + gamma_decision_count],
                matrices, point_indices, len(xy))
            gamma_error = decoded_gamma.detach().cpu().numpy() - initial_gamma_points
            print('[DOMAIN] gamma seed refit max_abs_error_rad='
                  f'{np.max(np.abs(gamma_error)):.9f}; '
                  f'seed_range={np.min(initial_gamma_points):.6f},{np.max(initial_gamma_points):.6f}; '
                  f'decoded_range={decoded_gamma.min().item():.6f},{decoded_gamma.max().item():.6f}',
                  flush=True)
        if self.project_posture_logits:
            decision = project_posture_decisions(decision, posture_decision_count, self.POSTURE_BOUND_EPS)
        ...
        def residual_fn(vector: torch.Tensor) -> torch.Tensor:
            ...
            domain_terms = None
            if self.neural_domain_weight:
                from .neural_domain import capture_inputs, domain_residual
                with capture_inputs(self.renderer) as actual_inputs:
                    rendered = (self.renderer(*render_args, gamma_points[render_indices])
                                if self.optimize_gamma else self.renderer(*render_args))
                domain_terms = domain_residual(torch.cat(actual_inputs), self.renderer.input_normalization)
                if self.hard_neural_domain:
                    from .neural_domain import domain_report
                    self._last_trial_domain_report = domain_report(
                        torch.cat(actual_inputs).detach().cpu().numpy(),
                        self.renderer.input_normalization)
                    self._trial_domain_valid = self._last_trial_domain_report['passed']
                    from .neural_domain import domain_margins
                    self._domain_margins = domain_margins(torch.cat(actual_inputs), self.renderer.input_normalization)
            else:
                rendered = (
            ...
            if domain_terms is not None:
                residuals.append(self.neural_domain_weight**0.5 * domain_terms)
        ...
        def finite_difference_jacobian(
            ...
            def augmented_residual(vector):
                result=residual_fn(vector)
                return torch.cat((result,self._domain_margins))
            if self.hard_neural_domain:
                baseline=augmented_residual(vector)
                domain_row_count=int(self._domain_margins.numel())
                differentiate=augmented_residual
            else:
                baseline=base_residual
                differentiate=residual_fn
            with torch.no_grad():
                for offset, column_tensor in enumerate(columns_to_evaluate):
                    ...
                    derivative = difference_column(differentiate, vector, column, step,
                                                   baseline, self.difference_scheme)
            ...
            full=torch.stack(columns,dim=1)
            if self.hard_neural_domain:
                self._domain_jacobian=full[-domain_row_count:]
                return full[:-domain_row_count]
            return full
        ...
        if self.hard_neural_domain and not np.isfinite(current_cost):
            raise ValueError(
                'decoded initialization failed actual neural domain; inversion not started; '
                f'domain_report={self._last_trial_domain_report}'
            )
        ...
        best_step = 0
        # Selection must retain the same explicit trajectory constraints used
        # during optimization, not undo them by returning an earlier MSE-only
        # winner. Preserve image-only selection for legacy unconstrained runs.
        regularized_checkpoint = any(weight > 0 for weight in (
            self.xy_target_skeleton_weight,
            self.xy_segment_length_weight,
            self.xy_segment_direction_weight,
            self.h_point_velocity_weight,
            self.h_point_acceleration_weight,
        ...
        selection_metric = (
            "regularized_cost_with_trajectory_constraints"
            if regularized_checkpoint
        ...
        for step in range(1, effective_max_steps + 1):
            ...
            with torch.no_grad():
                ...
                current_domain_margins=self._domain_margins.detach().clone() if self.hard_neural_domain else None
            print(
            ...
            try:
                if self.hard_neural_domain:
                    from .constrained_step import feasible_quadratic_step
                    result=feasible_quadratic_step(system.cpu().numpy(),gradient.cpu().numpy(),
                        current_domain_margins.cpu().numpy(),self._domain_jacobian.cpu().numpy(),
                        max_step=0.1)
                    delta=torch.as_tensor(result,device=gradient.device,dtype=gradient.dtype)
                else:
            ...
            if self.project_posture_logits:
                trial = project_posture_decisions(trial, posture_decision_count, self.POSTURE_BOUND_EPS)
            trial_cost = evaluate_cost(trial)
            domain_backtracks=0
            if self.hard_neural_domain and not np.isfinite(trial_cost):
                # Backtrack within the feasible domain. This changes optimizer
                # steps, never clips angles or mutates the forward renderer.
                for reduction in range(1,13):
                    domain_backtracks=reduction
                    trial=decision.detach().clone()
                    trial[active_columns] += delta.detach()*(0.5**reduction)
                    if self.project_posture_logits:
                        trial=project_posture_decisions(trial,posture_decision_count,self.POSTURE_BOUND_EPS)
                    trial_cost=evaluate_cost(trial)
                    if np.isfinite(trial_cost):
                        break
            if self.hard_neural_domain:
                actual_delta=(trial-decision)[active_columns]
                predicted_margins=current_domain_margins+self._domain_jacobian@actual_delta
                linearization_error=float(torch.max(torch.abs(predicted_margins-self._domain_margins)))
                print(f'[DOMAIN STEP {step:03d}] constraints={current_domain_margins.numel()} '
                      f'backtracks={domain_backtracks} feasible={np.isfinite(trial_cost)} '
                      f'proposed_norm={float(torch.linalg.vector_norm(delta)):.6g} '
                      f'actual_norm={float(torch.linalg.vector_norm(trial-decision)):.6g} '
                      f'trust_radius=0.1 max_margin_linearization_error={linearization_error:.6g}',flush=True)
        ...
        diagnostics: Dict[str, Any] = {
            ...
            "regularization": {
                "neural_domain_weight": self.neural_domain_weight,
                "hard_neural_domain": self.hard_neural_domain,
                "hard_domain_step": "linearized constrained LM with exact-forward feasibility backtracking",
                "hard_domain_logit_trust_radius": 0.1 if self.hard_neural_domain else None,
                "neural_domain_penalty": "actual dense network inputs; RMS and extrema; final strict audit required",
                "project_posture_logits": self.project_posture_logits,
                "difference_scheme": self.difference_scheme,
                "depth_spatial_weight": self.depth_spatial_weight,
                "depth_mm_per_pixel": self.depth_mm_per_pixel,
