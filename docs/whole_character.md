...
python -u tools/build_character_pairs.py \
  ...
  --output_npz data/processed/character_unet_clean_v4.npz \
  --chirography 楷 \
  --target_character 武 \
...
alignment filter: threshold=0.5500, rejected=...
```

如果 `--require_real_target` 导致字符样本过少，可以去掉它，让没有真实楷书目标的字符使用完整轨迹栅格目标。
程序会先根据图像边界估计纸张背景并归零，再对同字候选进行排序和目标配准。配准只改变监督目标，轨迹输入保持不动。低于 `0.55` 的真实配对不会进入 NPZ，详情写入同名 `.rejected.json`。
...
python -u tools/train_character.py \
  --config configs/default.yaml \
  --npz_path data/processed/character_unet_clean_v4.npz \
...
python -u tools/build_character_pairs.py \
  --config configs/default.yaml \
  --trajectory_csv data/raw/trajectories.csv \
  --output_npz data/processed/wu_unet_finetune_v4.npz \
  --character 武 \
  --target_character 武 \
  --target_image assets/targets/wu_kaishu_target.png \
  --trajectory_padding 16 \
...
python -u tools/train_character.py \
  --config configs/default.yaml \
  --npz_path data/processed/wu_unet_finetune_v4.npz \
...
python -u tools/build_character_pairs.py \
  --config configs/default.yaml \
  --trajectory_csv data/raw/trajectories.csv \
  --output_npz data/processed/yong_unseen_clean_v4.npz \
  --character 永 \
  --chirography 楷 \
  --require_real_target \
  --trajectory_padding 16 \
...
python -u tools/evaluate_character.py \
  --config configs/default.yaml \
  --npz_path data/processed/yong_unseen_clean_v4.npz \
  --checkpoint outputs/wu_unet_v3_finetune/character_best.pt \
  --output_dir outputs/yong_unseen_clean_v4 \
  --split all \
  --character 永 \
  --batch_size 1 \
  --num_images 20
